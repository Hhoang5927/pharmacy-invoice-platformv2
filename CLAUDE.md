...
| D5 | VAT allowed values are literally 0%, 5%, 8%, 10% | `TaxType` enum is STANDARD(10%)/REDUCED(5%)/EXEMPT(0%)/OTHER(0%) — no distinct slot for 8%, and `OTHER` silently defaults to a 0% rate | Flag — 8% VAT invoices could currently be miscalculated if classified as `OTHER`; worth an explicit fix decision |
...
| D7 | "Excel Dataset"/"Approved Dataset" appears throughout Business Rules as if it were a literal exported file automation reads from | Technical Specification (`TS-006`) clarifies "Approved Dataset" is really just an internal, JSON-serializable data contract passed between modules, not necessarily a literal `.xlsx` file; current design has automation reading the validated `PurchaseInvoice` directly from SQLite | Likely not a real conflict once reconciled with `TS-006`, but flag if Playwright automation design surfaces a genuine need for a literal file handoff |
