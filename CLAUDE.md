...
| D11 | Automation business rule implies one invoice is filled/saved atomically, all-or-nothing | **Resolved 2026-08 (PO-confirmed):** when a line item's medicine has genuinely exhausted every automated resolution option (every shortened-name candidate tried, and `create_medicine()` itself failed or still leaves it unfindable — surfaced as the new `MedicineUnresolvableError`, `infrastructure/automation/automation_errors.py`), `fill_and_save_invoice` now skips only that line and still saves the invoice with the remaining lines, instead of aborting the whole invoice. Added `InvoiceStatus.IMPORTED_NEEDS_MANUAL_LINE` (terminal, like `IMPORTED` — never auto-reprocessed, since re-running would create a duplicate invoice on-site) and `AutomationOutcome.manual_followup_items` (`domain/ports/services/browser_automation_provider.py`) to report exactly which line(s) the operator must finish by hand directly on the site. Deliberately narrow in scope: `UnitMismatchError`, row-settle `VerificationFailedError`, and the 180s ambiguous-match human-selection timeout are unchanged and still hard-abort the whole invoice — this safety net only ever applies to a line that was never entered in the first place, never to an entered value that failed verification (that rule is unchanged). No automation was added to reopen an already-saved site invoice and add the missing line — the operator completes that step by hand. **Amendment (2026-08, real "Coldi" incident, PO-reported):** this safety net had a real gap — a Playwright *timeout* on `create_medicine()`'s own `medicine.add_new_trigger` click was classified by `wrap_playwright_error` as `TransientInfrastructureError` (every Playwright timeout is, regardless of action), which `create_medicine()`'s `_run_outcome` deliberately re-raises rather than converting to a failure outcome — that type is not an `AutomationError`, so it escaped `_search_and_select_medicine_for_line`'s `except AutomationError` catch entirely and, with nothing actually retrying `fill_and_save_invoice` at its real call site (`composition_root.cli.run_automate` has no retry loop around it), hard-aborted the whole invoice instead of skipping just that line. Fixed narrowly in `_create_medicine_for_line`: it now catches `TransientInfrastructureError` from its own `create_medicine()` call and converts it to `AutomationError`, reaching `MedicineUnresolvableError` identically to a plain `create_medicine()` failure. Real end-to-end regression test added (`TestFillAndSaveInvoiceSkipsUnresolvableLine.test_middle_line_skipped_when_create_medicine_click_times_out`, a genuine Playwright timeout via an empty `#tblMain`, not simulated). Separately verified (real DOM evidence already on file, `medicine.add_new_trigger`'s own registry notes in `config/selector_registry.webnhathuoc.json`): despite that button's `ng-click="addDrugGroup()"` handler name, PO's direct DOM inspection already confirmed `#tblMain` is genuinely the create-medicine ("Thêm mới thuốc") dialog's own inner table, not a drug-group dialog — the selector itself is correct, not a second bug. The real "Coldi" root cause — why a medicine PO confirms genuinely exists on-site returns zero real search results even after the full 8s/33-check poll budget (see `logs/automation.log.2026-08-08`) — remains open and is NOT worked around by auto-creating on search failure (real duplicate-data risk, same class as the Nam Hà supplier incident); it needs its own root-cause fix, separate from this classification fix. **Amendment 2 (2026-08, PO decision — reverses the "NOT worked around by auto-creating" line above, explicit speed-over-correctness tradeoff):** PO has now explicitly decided the opposite: when a line's medicine search fails — either a genuine zero-result poll OR a Playwright *technical* failure while searching (`TransientInfrastructureError`, the same class Amendment 1 fixed for `create_medicine()`'s own click) — `_search_and_select_medicine_for_line` now calls `create_medicine()` immediately, no pause to ask, via the new `_fill_and_check_medicine_result_tolerant` wrapper (catches `TransientInfrastructureError` from the search step and treats it identically to a zero-result poll). Only if `create_medicine()` itself *also* fails does the line still fall through to `MedicineUnresolvableError`/manual follow-up, unchanged. PO has explicitly accepted the resulting real risk of creating a duplicate catalog entry for a medicine that in fact already exists but was merely unfindable by search (the exact "Coldi" scenario) — to be caught later by periodic manual catalog audit, not treated as a blocking correctness issue. This does not touch `create_medicine()`'s own failure handling (Amendment 1, unchanged) — only how a *search* failure is classified before deciding whether to attempt creation at all. Also (same change set): `domain.constants.MEDICINE_CODE_PREFIX` ("TH") is now configurable per run via `AppSettings.medicine_code_prefix`/`config/app_settings.default.toml`'s `[medicine].code_prefix` (default "TH", unchanged for every existing caller), threaded through `SqliteMedicineRepository`'s constructor and `PartyMatchingStep` into `MedicineValidationService.generate_next_medicine_code`'s new optional `prefix` param — since each pharmacy this system processes invoices for going forward is a separate, independent catalog (PO decision, not a redesign of the Business Rules' "TH1, TH2, TH3..." format itself, only which literal prefix string is used). | Closed |

| D12 | Not from `docs/` — a real-site UI detail with no documented business-rule statement either way | **Resolved 2026-08 (PO decision — reverses an earlier hold pending DOM evidence):** the invoice-import form has a SECOND, separate date field, "Ngày:" (`ng-model="noteDate"`, real DOM snapshot: `<div class="clickable-text daterange-picker text-link font-weight-bold"><input id="note-date-id" name="noteDate" type="text" ...><span class="ng-binding">09/08/2026</span></div>`) — distinct from "Ngày hóa đơn:" (`invoice.date_field`/`invoice.date_calendar_trigger`), already flagged as a separate field by that entry's own registry notes before this snapshot existed. Previously left at the site's own today-by-default value; PO now requires it filled with the invoice's real date too, since this system processes OLD invoices spanning ~3 years and leaving it at the automation run date would make every backdated invoice look like same-day bookkeeping. New registry entries `invoice.note_date_trigger` (`div.daterange-picker:has(#note-date-id)`) and `invoice.note_date_field` (`#note-date-id`), both `status: "confirmed"` on the strength of the real DOM snapshot for the ELEMENT itself — but two things are inferred, not independently re-confirmed against the live site (flagged in that entry's own notes for the next real dry-run): (1) that this element opens the SAME shared `invoice.date_calendar_dropdown` bootstrap-datepicker widget as "Ngày hóa đơn:" (inferred from the matching `daterange-picker` class name); (2) that the click target is the WRAPPER div, not the `<input>` itself (chosen because the 2 independently-recorded real click sequences for this exact field, `05_full_flow_existing_supplier_medicine.py:41` and `06_multi_line_items.py:21`, both clicked the field's own rendered TEXT via `get_by_text`, never an input directly — consistent with `invoice.date_field`'s own history of an input that looks fillable but is display-only). `PlaywrightBrowserAutomationProvider._select_date_via_calendar` (generalized from the invoice-date-only `_select_invoice_date_via_calendar`, parameterized by trigger key) is reused as-is for both fields — the year/month/day navigation entries are page-shared and unchanged. `_fill_note_date_and_verify` mirrors `_fill_invoice_date_and_verify`'s own roundtrip-verification pattern. Real end-to-end tests added (`TestNoteDateFilledWithRealInvoiceDate`, including a multi-year-back case) against a fixture extension modeling both fields bound to one shared calendar (`activeDateInputId` tracking which trigger opened it, matching `invoice.date_calendar_dropdown`'s own already-confirmed "repositions to whichever trigger was last activated" behavior). | Closed |
